import seed from "../database/schema/familyActivitySeed.json"
...
export function createFamilyActivityStore() {
  ...
  return {
    addChore({ assignedBy, assignedTo, dueLabel, rewardStars, title }) {
      const chore = {
        id: crypto.randomUUID(),
        title,
        assignedTo,
    ...
    addDinnerIdea({ createdBy, groceryHint, title }) {
      const idea = {
        id: crypto.randomUUID(),
        title,
        createdBy,
    ...
    addGroceryItem({ addedBy, category, name }) {
      const item = {
        id: crypto.randomUUID(),
        name,
        category,
    ...
    addRequest({ body, createdBy, title }) {
      const request = {
        id: crypto.randomUUID(),
        title,
        body,
    ...
    getOverview(user) {
      return {
        ...
        stats: {
          ...
          topDinnerVotes:
            store.dinnerIdeas.reduce(
              (maxVotes, entry) => Math.max(maxVotes, entry.votes.length),
              0,
            ) || 0,
        },
      }
    ...
    toggleGroceryItem(id, actorName) {
